...
class vnT_FTP 
{
	 ...
	 * 		<pre>  $ftp = &vnT_FTP::getInstance($host);</pre>
	 *
	 ...
	 * @return	vnT_FTP	The FTP Client object.
	 * @since 1.5
	 */
	function &getInstance($host = '127.0.0.1', $port = '21', $options = null, $user = null, $pass = null)
	{
		...
		if (!isset ($instances[$signature]) || !is_object($instances[$signature])) {
			$instances[$signature] = new vnT_FTP($options);
		} else {
	...
	function connect($host = '127.0.0.1', $port = 21) {
		...
		if (FTP_NATIVE) {
			...
			if ($this->_conn === false) {
				$this->error[] = 'vnT_FTP::connect: Could not connect to host "'.$host.'" on port '.$port;
				return false;
			}
		...
		if (!$this->_verifyResponse(220)) {
			$this->error[] = 'vnT_FTP::connect: Bad response' ;
			return false;
		}
	 ...
	 * @return boolean True if successful
	 */
	function login($user = 'root', $pass = '123456') {
		global $vnT,$conf,$func;	
		// If native FTP support is enabled lets use it...
		if (FTP_NATIVE) {
			if (@ftp_login($this->_conn, $user, $pass) === false) {
				$this->error[] = 'vnT_FTP::login: Unable to login' ;
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('USER '.$user, array(331, 503))) {
			$this->error[] = 'vnT_FTP::login: Bad Username' . 'Server response: '.$this->_response.' [Expected: 331] Username sent: '.$user  ;
			return false;
		}
		...
		if (!$this->_putCmd('PASS '.$pass, 230)) {
			$this->error[] = 'vnT_FTP::login: Bad Password' . 'Server response: '.$this->_response.' [Expected: 230] Password sent: '.str_repeat('*', strlen($pass)) ;
			return false;
		}
	...
	function pwd() {
		...
		if (FTP_NATIVE) {
			if (($ret = @ftp_pwd($this->_conn)) === false) {
				$this->error[] = 'vnT_FTP::pwd: Bad response' ;	
				return false;
			}
			return $ret;
		}
		...
		if (!$this->_putCmd('PWD', 257)) {
			$this->error[] = 'vnT_FTP::pwd: Bad response' . 'Server response: '.$this->_response.' [Expected: 257]' ;	
	...
	function syst() {
		...
		if (FTP_NATIVE) {
			if (($ret = @ftp_systype($this->_conn)) === false) {
				$this->error[] = $func->html_err('vnT_FTP::syst: Bad response' );	
				return false;
			}
		} else {
			// Send print working directory command and verify success
			if (!$this->_putCmd('SYST', 215)) {
				$this->error[] = 'vnT_FTP::syst: Bad response' . 'Server response: '.$this->_response.' [Expected: 215]' ;	
				return false;
			}
			$ret = $this->_response;
		}
	...
	function chdir($path) {
		...
		if (FTP_NATIVE) {
			if (@ftp_chdir($this->_conn, $path) === false) {
				$this->error[]= 'vnT_FTP::chdir: Bad response';
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('CWD '.$path, 250)) {
			$this->error[]= 'vnT_FTP::chdir: Bad response' . 'Server response: '.$this->_response.' [Expected: 250] Path sent: '.$path;
			return false;
		}
	...
	function reinit() {
		...
		if (FTP_NATIVE) {
			if (@ftp_site($this->_conn, 'REIN') === false) {
				$this->error[]= 'vnT_FTP::reinit: Bad response';
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('REIN', 220)) {
			$this->error[]= 'vnT_FTP::reinit: Bad response' . 'Server response: '.$this->_response.' [Expected: 220]' ;
			return false;
		}
	...
	function rename($from, $to) {
		...
		if (FTP_NATIVE) {
			if (@ftp_rename($this->_conn, $from, $to) === false) {
				$this->error[]= 'vnT_FTP::rename: Bad response' ;
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('RNFR '.$from, 350)) {
			$this->error[]= 'vnT_FTP::rename: Bad response'. 'Server response: '.$this->_response.' [Expected: 320] From path sent: '.$from;
			return false;
		}

		// Send rename to command to the server
		if (!$this->_putCmd('RNTO '.$to, 250)) {
			$this->error[]=  'vnT_FTP::rename: Bad response'. 'Server response: '.$this->_response.' [Expected: 250] To path sent: '.$to ;
			return false;
		}
	...
	function chmod($path, $mode) {
		...
		if (FTP_NATIVE) {
			if (@ftp_site($this->_conn, 'CHMOD '.$mode.' '.$path) === false) {
				if($this->_OS != 'WIN') {
					$this->error[]= 'vnT_FTP::chmod: Bad response' ;
				}
				return false;
			}
			return true;
		...
		if (!$this->_putCmd('SITE CHMOD '.$mode.' '.$path, array(200, 250))) {
			if($this->_OS != 'WIN') {
				$this->error[]= 'vnT_FTP::chmod: Bad response'. 'Server response: '.$this->_response.' [Expected: 200 or 250] Path sent: '.$path.' Mode sent: '.$mode;
			}
			return false;
		}
		return true;
	...
	function delete($path) {
		...
		if (FTP_NATIVE) {
			if (@ftp_delete($this->_conn, $path) === false) {
				if (@ftp_rmdir($this->_conn, $path) === false) {
					$this->error[]= 'vnT_FTP::delete: Bad response';
					return false;
				}
			}
			return true;
		...
		if (!$this->_putCmd('DELE '.$path, 250)) {
			if (!$this->_putCmd('RMD '.$path, 250)) {
				$this->error[]= 'vnT_FTP::delete: Bad response' . 'Server response: '.$this->_response.' [Expected: 250] Path sent: '.$path ;
				return false;
			}
		}
		return true;
	...
	function mkdir($path) {
		...
		if (FTP_NATIVE) {
			if (@ftp_mkdir($this->_conn, $path) === false) {
				$this->error[]= 'vnT_FTP::mkdir: Bad response' ;
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('MKD '.$path, 257)) {
			$this->error[]= 'vnT_FTP::mkdir: Bad response' . 'Server response: '.$this->_response.' [Expected: 257] Path sent: '.$path ;
			return false;
		}
		return true;
	}
	...
	function restart($point) {
		...
		if (FTP_NATIVE) {
			if (@ftp_site($this->_conn, 'REST '.$point) === false) {
				$this->error[]= 'vnT_FTP::restart: Bad response' ;
				return false;
			}
			return true;
		}
		...
		if (!$this->_putCmd('REST '.$point, 350)) {
			$this->error[]= 'vnT_FTP::restart: Bad response' . 'Server response: '.$this->_response.' [Expected: 350] Restart point sent: '.$point ;
			return false;
		}
	...
	function create($path) {
		...
		if (FTP_NATIVE) {
			...
			if (@ftp_pasv($this->_conn, true) === false) {
				$this->error[]= 'vnT_FTP::create: Unable to use passive mode' ;
				return false;
			}

			$buffer = fopen('buffer://tmp', 'r');
			if (@ftp_fput($this->_conn, $path, $buffer, FTP_ASCII) === false) {
				$this->error[]= 'vnT_FTP::create: Bad response' ;
				fclose($buffer);
				return false;
			}
			fclose($buffer);
		...
		if (!$this->_putCmd('STOR '.$path, array (150, 125))) {
			...
			$this->error[]= 'vnT_FTP::create: Bad response'. 'Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$path ;
			return false;
		}
		...
		if (!$this->_verifyResponse(226)) {
			$this->error[]= 'vnT_FTP::create: Transfer Failed'. 'Server response: '.$this->_response.' [Expected: 226] Path sent: '.$path ;
			return false;
		}
	...
	function read($remote, &$buffer) {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				$this->error[]= 'vnT_FTP::read: Unable to use passive mode' ;
				return false;
			}
			...
			if (@ftp_fget($this->_conn, $tmp, $remote, $mode) === false) {
				fclose($tmp);
				$this->error[]='vnT_FTP::read: Bad response';
				return false;
			}
		...
		if (!$this->_putCmd('RETR '.$remote, array (150, 125))) {
			@ fclose($this->_dataconn);
			$this->error[]=  'vnT_FTP::read: Bad response'. 'Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$remote ;
			return false;
		}
		...
		if (!$this->_verifyResponse(226)) {
			$this->error[]= 'vnT_FTP::read: Transfer Failed' . 'Server response: '.$this->_response.' [Expected: 226] Path sent: '.$remote ;
			return false;
		}
	...
	function get($local, $remote) {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				$this->error[]= 'vnT_FTP::get: Unable to use passive mode';
				return false;
			}

			if (@ftp_get($this->_conn, $local, $remote, $mode) === false) {
				$this->error[]= 'vnT_FTP::get: Bad response' ;
				return false;
			}
			return true;
		}
		...
		$fp = fopen($local, "wb");
		if (!$fp) {
			$this->error[]=  'vnT_FTP::get: Unable to open local file for writing'. 'Local path: '.$local ;
			return false;
		}
		...
		if (!$this->_putCmd('RETR '.$remote, array (150, 125))) {
			@ fclose($this->_dataconn);
			$this->error[]= 'vnT_FTP::get: Bad response' . 'Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$remote;
			return false;
		}
		...
		if (!$this->_verifyResponse(226)) {
			$this->error[]= 'vnT_FTP::get: Transfer Failed' . 'Server response: '.$this->_response.' [Expected: 226] Path sent: '.$remote ;
			return false;
		}
	...
	function store($local, $remote = null) {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				$this->error[]= 'vnT_FTP::store: Unable to use passive mode' ;
				return false;
			}

			if (@ftp_put($this->_conn, $remote, $local, $mode) === false) {
				$this->error[]= 'vnT_FTP::store: Bad response' ;
				return false;
			}
			return true;
		}
		...
		if (@ file_exists($local)) {
			$fp = fopen($local, "rb");
			if (!$fp) {
				$this->error[]= 'vnT_FTP::store: Unable to open local file for reading'. 'Local path: '.$local ;
				return false;
			}
		} else {
			$this->error[]= 'vnT_FTP::store: Unable to find local path'. 'Local path: '.$local ;
			return false;
		}
		...
		if (!$this->_putCmd('STOR '.$remote, array (150, 125))) {
			@ fclose($fp);
			@ fclose($this->_dataconn);
			$this->error[]= 'vnT_FTP::store: Bad response'. 'Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$remote ;
			return false;
		}
		...
		while (!feof($fp)) {
			...
			do {
				if (($result = @ fwrite($this->_dataconn, $line)) === false) {
					$this->error[]= 'vnT_FTP::store: Unable to write to data port socket' ;
					return false;
				}
		...
		if (!$this->_verifyResponse(226)) {
			$this->error[]=  'vnT_FTP::store: Transfer Failed' . 'Server response: '.$this->_response.' [Expected: 226] Path sent: '.$remote ;
			return false;
		}
	...
	function write($remote, $buffer) {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				$this->error[]= 'vnT_FTP::write: Unable to use passive mode' ;
				return false;
			}
			...
			if (@ftp_fput($this->_conn, $remote, $tmp, $mode) === false) {
				fclose($tmp);
				$this->error[]= 'vnT_FTP::write: Bad response' ;
				return false;
			}
			fclose($tmp);
			return true;
		...
		// Send store command to the FTP server
		if (!$this->_putCmd('STOR '.$remote, array (150, 125))) {
			$this->error[]= 'vnT_FTP::write: Bad response' . 'Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$remote ;
			@ fclose($this->_dataconn);
			return false;
		}
		...
		do {
			if (($result = @ fwrite($this->_dataconn, $buffer)) === false) {
				$this->error[]= 'vnT_FTP::write: Unable to write to data port socket' ;
				return false;
			}
		...
		if (!$this->_verifyResponse(226)) {
			$this->error[]=  'vnT_FTP::write: Transfer Failed' . 'Server response: '.$this->_response.' [Expected: 226] Path sent: '.$remote ;
			return false;
		}
	...
	function listNames($path = null) {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				$this->error[]=  'vnT_FTP::listNames: Unable to use passive mode' ;
				return false;
			}
			...
			if (($list = @ftp_nlist($this->_conn,$path)) === false) {
				...
				$this->error[]= 'vnT_FTP::listNames: Bad response' ;
				return false;
			}
		...
		if (!$this->_putCmd('NLST'.$path, array (150, 125))) {
			...
			$this->error[]= 'vnT_FTP::listNames: Bad response' . 'Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$path ;
			return false;
		}
		...
		// Everything go okay?
		if (!$this->_verifyResponse(226)) {
			$this->error[]= 'vnT_FTP::listNames: Transfer Failed' . 'Server response: '.$this->_response.' [Expected: 226] Path sent: '.$path ;
			return false;
		}
	...
	function listDetails($path = null, $type = 'all') {
		...
		if (FTP_NATIVE) {
			// turn passive mode on
			if (@ftp_pasv($this->_conn, true) === false) {
				$this->error[]= 'vnT_FTP::listDetails: Unable to use passive mode' ;
				return false;
			}

			if (($contents = @ftp_rawlist($this->_conn, $path)) === false) {
				$this->error[]= 'vnT_FTP::listDetails: Bad response' ;
				return false;
			}
		} else {
			// Non Native mode
			...
			if (!$this->_putCmd(($recurse == true) ? 'LIST -R' : 'LIST'.$path, array (150, 125))) {
				$this->error[]= 'vnT_FTP::listDetails: Bad response' . 'Server response: '.$this->_response.' [Expected: 150 or 125] Path sent: '.$path ;
				@ fclose($this->_dataconn);
				return false;
			}
			...
			// Everything go okay?
			if (!$this->_verifyResponse(226)) {
				$this->error[]= 'vnT_FTP::listDetails: Transfer Failed' . 'Server response: '.$this->_response.' [Expected: 226] Path sent: '.$path ;
				return false;
			}
		...
		if (!$osType) {
			$this->error[]= 'vnT_FTP::listDetails: Unrecognized directory listing format' ;
			return false;
		}
	...
	function _putCmd($cmd, $expectedResponse) {
		...
		if (!is_resource($this->_conn)) {
			$this->error[]= 'vnT_FTP::_putCmd: Not connected to the control port' ;
			return false;
		}

		// Send the command to the server
		if (!fwrite($this->_conn, $cmd."\r\n")) {
			$this->error[]= 'vnT_FTP::_putCmd: Unable to send command: '.$cmd ;
		}
	...
	function _verifyResponse($expected) {
		...
		if (!isset($parts[1])) {
			$this->error[]= 'vnT_FTP::_verifyResponse: Timeout or unrecognized response while waiting for a response from the server' . 'Server response: '.$this->_response;
			return false;
		}
	...
	function _passive() {
		...
		// Make sure we have a connection to the server
		if (!is_resource($this->_conn)) {
			$this->error[]='vnT_FTP::_passive: Not connected to the control port' ;
			return false;
		}
		...
		// Catch a timeout or bad response
		if (!isset($parts[1])) {
			$this->error[]= 'vnT_FTP::_passive: Timeout or unrecognized response while waiting for a response from the server' . 'Server response: '.$this->_response;
			return false;
		}
		...
		if ($this->_responseCode != '227') {
			$this->error[]= 'vnT_FTP::_passive: Unable to obtain IP and port for data transfer' . 'Server response: '.$this->_responseMsg;
			return false;
		}

		// Snatch the IP and port information, or die horribly trying...
		if (preg_match('~\((\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d+))\)~', $this->_responseMsg, $match) == 0) {
			$this->error[]= 'vnT_FTP::_passive: IP and port for data transfer not valid' . 'Server response: '.$this->_responseMsg;
			return false;
		}
		...
		if (!$this->_dataconn) {
			$this->error[]= 'vnT_FTP::_passive: Could not connect to host '.$this->_pasv['ip'].' on port '.$this->_pasv['port'].'.  Socket error number '.$errno.' and error message: '.$err ;
			return false;
		}
	...
	function _mode($mode) {
		if ($mode == FTP_BINARY) {
			if (!$this->_putCmd("TYPE I", 200)) {
				$this->error[]= 'vnT_FTP::_mode: Bad response' . 'Server response: '.$this->_response.' [Expected: 200] Mode sent: Binary' ;
				return false;
			}
		} else {
			if (!$this->_putCmd("TYPE A", 200)) {
				$this->error[]= 'vnT_FTP::_mode: Bad response'. 'Server response: '.$this->_response.' [Expected: 200] Mode sent: Ascii' ;
				return false;
			}
		}
		return true;
