...
( function() {
	var imageDialog = function( editor, dialogType ) {
			...
			return {
				...
				minHeight: 360,
				onShow: function() {
					this.wrapElement = false;
					this.imageElement = false;
					this.linkElement = false;
					this.shortElement = false;

					// Default: create a new element.
					this.imageEditMode = false;
					this.linkEditMode = false;
					this.shortEditMode = false;
					...
					var editor = this.getParentEditor(),
						...
						loader = CKEDITOR.document.getById( imagePreviewLoaderId );
						if(element){
							// var short = element.getNextSourceNode();
							// var short = element && editor.elementPath( element );
							var short = element.getNext();
							if(short == null){
								if(link){
									short = link.getNext();
									if(short == null){
					...
					if(short != ""){
						if(short.$.className != 'ck_desc_img'){
							short = "";
						}
					}
					// Hide loader.
					if ( loader )
						loader.setStyle( 'display', 'none' );
					this.wrapElement = editor.document.createElement( 'div' );
					...
					if ( short ) {
						this.shortElement = short;
						this.shortEditMode = true;
						if ( dialogType == 'image' )
							this.setupContent( SHORT, short );
					}
				...
				onOk: function() {
					...
					if ( this.imageEditMode ) {
						...
						// ImageButton dialog and Image element.
							...
							// editor.insertElement( this.imageElement );
							editor.insertElement(this.wrapElement);
							this.wrapElement.append(this.imageElement);
							this.wrapElement.append( this.shortElement );
						} else {
					...
					if ( !this.shortEditMode ){
						this.shortElement = editor.document.createElement( 'span' );
						this.shortElement.addClass("ck_desc_img");
					}
					// Set attributes.
					this.commitContent( IMAGE, this.imageElement );
					this.commitContent( LINK, this.linkElement );
					this.commitContent( SHORT, this.shortElement );
					...
					if ( !this.imageEditMode ) {
						if ( this.addLink ) {
							if ( !this.linkEditMode ) {
								// Insert a new link.
								editor.insertElement(this.wrapElement);
								this.wrapElement.append( this.linkElement );
								this.linkElement.append( this.imageElement, false );
								this.wrapElement.append( this.shortElement, false );
							} else {
					...
					// Image already exists.
						...
						// Remove Link, Image exists.
							...
							// editor.getSelection().selectElement( this.wrapElement);
							// editor.insertElement(this.wrapElement);
							// this.wrapElement.append(this.imageElement);
							// this.wrapElement.append( this.shortElement );
						}else if(!this.shortEditMode){
							editor.insertElement(this.wrapElement);
							this.wrapElement.append(this.imageElement);
							this.wrapElement.append( this.shortElement );
						}
				...
				contents: [ {
					...
					},
					{
						id: 'txtAlt',
						type: 'text',
						label: editor.lang.image.alt+' (alt)',
						accessKey: 'T',
						'default': '',
						onChange: function() {
							...
							children: [
								{
									...
									setup: function( type, element ) {
										if ( type == IMAGE){
											this.setValue( element.getAttribute( 'data-ck-zoom' ) || 'yes' );
										}
									},
									commit: function( type, element) {
										if ( type == IMAGE){
											element.setAttribute( 'data-ck-zoom', this.getValue() );
										}
									}
								},
								{
