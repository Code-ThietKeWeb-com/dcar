...
/**
 * Class manage sesssion cokkie
 * @version : 1.0
 * @date upgrade : 09/01/2009 by Thai Son
 */

include (dirname(__FILE__).DIRECTORY_SEPARATOR.'storage.php');
...
class vnT_Session
{
	...
	*/	
	...
	function __construct( $store = 'none', $options = array() )
	{
		...
		$this->_store =& SESessionStorage::getInstance($store, $options);
	 ...
	 * 		<pre>  $session = &vnT_Session::getInstance();</pre>
	 *
	 * @access	public
	 * @return	vnT_Session	The Session object.
	 * @since	1.5
	 */
	function & getInstance($handler, $options)
	{
		...
		if (!is_object($instance)) {
			$instance = new vnT_Session($handler, $options);
		}
	...
	*/
	 ...
	 */
	function &get($name, $default = null, $namespace = 'trustvn')
	{
	 ...
	 */
	function set($name, $value, $namespace = 'trustvn')
	{
		$namespace = '__'.$namespace; //add prefix to namespace to avoid collisions
	...
	*/
	function has( $name, $namespace = 'trustvn' )
	{
		$namespace = '__'.$namespace; //add prefix to namespace to avoid collisions
	...
	*/
	function clear( $name, $namespace = 'trustvn' )
	{
		$namespace = '__'.$namespace; //add prefix to namespace to avoid collisions
	...
	*/
	 ...
	 * to call vnT_Session::close(),but as session data is locked to prevent concurrent
