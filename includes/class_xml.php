...
/*======================================================================*\
|| #################################################################### ||
|| # vBulletin 3.5.4 - Licence Number VBFJN5HF35
|| # ---------------------------------------------------------------- # ||
...
class XMLparser
{
   ...
   */
  function &parse ($encoding = 'uft-8', $emptydata = true)
  {
  ...
  function add_node (&$children, $name, $value)
  {
    if (! is_array($children) or ! in_array($name, array_keys($children))) { // not an array or its not currently set
      ...
      if (is_array($children[$name]) and isset($children[$name][0])) { // its the same tag and is already an array
        $children[$name][] = $value;
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: 14:17, Sun Feb 26th 2006
|| # CVS: $RCSfile: class_xml.php,v $ - $Revision: 1.22 $
|| ####################################################################
\*======================================================================*/
?>