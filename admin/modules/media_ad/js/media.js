...
var LFILE = {
  reload : function( path, file ){
    $("div#imglist").html(vnt_loading_data).load("index.php?mod=media&act=remote&do=gallery&path=" + path + "&imgfile=" + file + "&type=image"  + "&random=" + randomNum(10)) ;
  },
  setSelFile : function(){
  ...
  $.ajax({
    ...
    success : function(d) {
      ...
      if (e[0] == "ERROR") {
        ...
        $("#imgfolder").load(remote_url + "folder_gallery&path=" + h + "&folder=" + d + "&random=" + randomNum(10));
        LFILE.reload(path,'');
      }
    }
  });
...
var vnTUpload = {
  ...
  renderUI: function(){
    ...
    $('#upload-queue').html('<div class="queue-header">' +
      ...
      '<div class="row">' +
      '<div class="col-xs-4">' + LANG.file_name + '</div>' +
      '<div class="col-xs-3">' + LANG.altimage + '</div>' +
      '<div class="col-xs-2">' + LANG.upload_size + '</div>' +
      '<div class="col-xs-2">' + LANG.upload_status + '</div>' +
  ...
  updateList: function(){
    ...
    $.each( vnTUpload.uploader.files, function(i, file){
      ...
      fileList.append(
        ...
        '<div class="col-xs-4 file-name"><span>' + file.name + '</span></div>' +
        '<div class="col-xs-3 file-alt"><input type="text" value="' + fileAlt + '" onkeyup="NVLDATA.setValue( \'' + file.id + '\', this.value);" class="form-control upload-file-alt dynamic"/></div>' +
        '<div class="col-xs-2 file-size">' + plupload.formatSize(file.size) + '</div>' +
        '<div class="col-xs-2 file-status">' + file.percent + '%</div>' +
        '<div class="col-xs-1 file-action text-right"></div>' +
        '</div>'
      );
  ...
  init : function(){
    ...
    vnTUpload.uploader = new plupload.Uploader({
      ...
      url :  'modules/media_ad/ajax/upload_gallery.php?folder_path='+folderPath,
