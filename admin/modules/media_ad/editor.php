...
class vntModule
{
  ...
  function vntModule ()
  {
    ...
    $this->skin = new XiTemplate(DIR_MODULE . DS . "media_ad" . DS . "html" . DS . "editor.tpl");
    ...
    $this->skin->assign('DIR_IMAGE_MEDIA', DIR_IMAGE_MEDIA); 
		...
		switch ($_GET['do']) {
			...
			case "insert_img" : $jout =  $this->do_InsertImg();  break;	   
	...
	function do_folderList ()
	{
		...
		$check_allow_upload_dir = check_allow_upload_dir( $path );		
		$data = array();
	...
	function do_imgList ()
	{
		...
		if($num = $vnT->DB->num_rows($result))
		{
			while($file = $vnT->DB->fetch_row($result))
			{
				...
				$file['sel'] = ( $selectfile == $file['file_name'] ) ? " imgsel" : "";
	...
	function do_InsertImg ()
	{
		...
		$pic_name = string_to_filename( basename($_POST['pic_name'] ) ) ;
		$pic_alt = $_POST['pic_alt'];		
		$pic_width = (int)$_POST['pic_width']; 
		$arr_json['ok'] = 0;
		$arr_json['mess'] = '';
		
		//check
		$create_new = 0;
		$res_ck = $vnT->DB->query("SELECT * FROM media_files WHERE file_id=".$file_id);
		if($row_ck = $vnT->DB->fetch_row($res_ck))
		{ 
			$folder_id = $row_ck['folder_id'];
			
			$path = substr($row_ck['file_src'], 0, strrpos($row_ck['file_src'], "/"));
			$pic_name_old = string_to_filename (substr($row_ck['file_name'], 0, strrpos($row_ck['file_name'], ".")));
 	
			if($pic_name_old != $pic_name){
				$create_new= 1;	
			}
			if($pic_width != $row_ck['file_width']){
				$create_new= 1;	
			}
			...
			{ 
				...
				$path_file_desc =   DIR_MEDIA."/".$row_ck['file_src'];
				$up['path'] = DIR_MEDIA."/";
				$up['dir'] = $path;
				$up['path_file_desc'] = $path_file_desc;
				$up['type'] = "hinh";					 
				$up['pic_name'] = $pic_name;					
				$up['w'] = $pic_width;
				$up['thum'] = 1; 
				$up['w_thum'] = $vnT->setting['upload_thum_width'];
			  
				$result = $vnT->File->ResizeImage($up);								 
				if (empty($result['err'])) {
					$file_name = $result['link'];
					...
					$error = $func->html_err($result['err']);
				} 
				...
				if(empty($error))
				{
					...
					$link_file = ROOT_URI."vnt_upload/".$file_src;
					$arr_json['ok'] = 1;
				...
				$link_file = ROOT_URI."vnt_upload/".$row_ck['file_src'];
				$arr_json['ok'] = 1;
				$arr_json['link_file'] = $link_file;
			}
			...
			$arr_json['ok'] = 0;
			$arr_json['mess'] = $vnT->lang['errorNotFindFile'] ;
		}
