...
class vntModule
{
  ...
  function vntModule()
  {
    ...
    switch ($vnT->input['sub']) {
      case 'add':
        $nd['f_title'] = "Thêm quận huyện mới";
        $nd['content'] = $this->do_Add($lang);

        break;
      case 'edit':
        $nd['f_title'] = "Cập nhật quận huyện";
  ...
  function do_Add($lang)
  {
    ...
    if (!empty($_POST['do_submit'])) {
      ...
      // Check for Error
      $res_chk = $DB->query("SELECT * FROM iso_states  WHERE code='{$code}' and city='$city' ");
      ...
      if (empty($err)) {
        ...
        $kq = $DB->do_insert("iso_states", $cot);
        if ($kq) {
          $state = $DB->insertid();
          ...
          //insert adminlog
          $func->insertlog("Add", $_GET['act'], $state);
  ...
  function do_Edit($lang)
  {
    ...
    if (isset($_POST['do_submit'])) {
      ...
      $name = $func->txt_HTML($_POST['name']);
      // Check for Error
      $res_chk = $DB->query("SELECT * FROM iso_states  WHERE code='{$code}' and id<>$id and city='$city'");
      if ($check = $DB->fetch_row($res_chk))
        $err = $func->html_err("Code name tồn tại");
      ...
      if (empty($err)) {
        ...
        $cot['name'] = $name;
        $cot['name_en'] = $vnT->input['name_en'];
        $kq = $DB->do_update("iso_states", $cot, "id=$id");
        if ($kq) {
    ...
    $query = $DB->query("SELECT * FROM iso_states WHERE id=$id ");
    if ($data = $DB->fetch_row($query)) {
      $data['name'] = $func->txt_unHTML($data['name']);
    }
    $data['list_city'] = List_City($data['city']);
  ...
  function do_Del($lang)
  {
    ...
    if(empty($vnT->input['csrf_token']) || ($vnT->input['csrf_token'] != $_SESSION['vnt_csrf_token']) ) {
      ...
      $query = 'DELETE FROM iso_states WHERE id IN (' . $ids . ')';
  ...
  function render_row($row_info, $lang)
  {
    ...
    $output['order'] = $row['ext'] . "<input name=\"txt_Order[{$id}]\" type=\"text\" size=\"2\" maxlength=\"2\" style=\"text-align:center\" value=\"{$row['s_order']}\"  onkeypress=\"return is_num(event,'txtOrder')\" onchange='javascript:do_check($id)' />";
  ...
  function do_Manage($lang)
  {
        ...
        switch ($_POST["do_action"]) {
          case "do_edit":
            ...
            for ($i = 0; $i < count($h_id); $i++) {
              ...
              $ok = $DB->do_update("iso_states", $dup, "id=" . $h_id[$i]);
              if ($ok) {
                $str_mess .= $h_id[$i] . ", ";
              }
            }
    ...
    if ((int)$vnT->input["do_display"]) {
      ...
      if(empty($vnT->input['csrf_token']) || ($vnT->input['csrf_token'] != $_SESSION['vnt_csrf_token']) ) {
        ...
        unset($_SESSION['vnt_csrf_token']);
        $ok = $DB->query("Update iso_states SET display=1 WHERE id=" . $vnT->input["do_display"]);
        if ($ok) {
    ...
    if ((int)$vnT->input["do_hidden"]) {
      if(empty($vnT->input['csrf_token']) || ($vnT->input['csrf_token'] != $_SESSION['vnt_csrf_token']) ) {
        ...
        $ok = $DB->query("Update iso_states SET display=0 WHERE id=" . $vnT->input["do_hidden"]);
        if ($ok) {
    ...
    $query = $DB->query("SELECT * FROM iso_states $where ");
    ...
    $table['title'] = array(
      ...
      'id' => "ID quận huyện|15%|center",
      'name' => "Tên quận huyện (VN)||left",
      'name_en' => "Tên tỉnh thành (EN)|25%|left",
      's_price' => "Phí vận chuyển|15%|center",
      'action' => "Action|10%|center");
    $sql = "SELECT * FROM iso_states  $where ORDER BY s_order ASC , name ASC  LIMIT $start,$n";
    //print "sql = ".$sql."<br>";
    $result = $DB->query($sql);
    if ($DB->num_rows($result)) {
      ...
      $table['row'] = array();
      $table['extra'] = "<div align=center class=font_err >Chưa có quận huyện nào</div>";
    }
    ...
    $data['list_city'] = List_City($city, "onChange='submit();'");
    $data['keyword'] = $keyword;
    $data['list_search'] = $this->List_Search($search);
    $data['list_display'] = List_Display($display, "onChange='submit();'");
    $data['link_excel'] = $conf['rooturl']."admin/modules/country_ad/excel_state.php?lang=$lang".$ext ;
    $data['totals'] = $totals;
    $data['err'] = $err;
    $data['nav'] = $nav;
    /*assign the array to a template variable*/
  ...
  function List_Search($did)
  {
    ...
    if ($did == "code")
      $text .= "<option value=\"code\" selected> Mã quận huyện </option>";
    else
      $text .= "<option value=\"code\" >  Mã quận huyện </option>";
    if ($did == "name")
      $text .= "<option value=\"name\" selected> Tên quận huyện </option>";
    else
      $text .= "<option value=\"name\">Tên quận huyện</option>";
    $text .= "</select>";
    return $text;
  }
