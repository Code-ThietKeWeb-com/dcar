...
class vntModule extends Model
{
  ...
  function vntModule ()
  {
    ...
    switch ($vnT->input['sub']) {
      case 'add':
          $nd['f_title'] = $vnT->lang['add_pos'];
          $nd['content'] = $this->do_Add($lang);
      break;
      case 'edit':
          $nd['f_title'] = $vnT->lang['edit_pos'];
      ...
      break;
      default:
          $nd['f_title'] = $vnT->lang['manage_pos'];
  ...
  function do_Add ($lang)
  {
    ...
    if ($vnT->input['do_submit'] == 1) {
      ...
      $name = $vnT->input['name'];
      // Check for Error
      $query = $DB->query("SELECT name FROM ad_pos  WHERE name='{$name}' ");
