...
*/
// exit("dsadsadsada");
define('THUMB_CACHE',           './vnt_upload/thumbs/cache/');    // Path to cache directory (must be writeable)
...
define('PNG_QUALITY',          90);           // Quality of generated JPEGs (0 - 100; 100 being best)
...
if (!file_exists($file_temp)) {
    ...
    if ($ignore && $type == 1) {
        if (preg_match('/\x00\x21\xF9\x04.{4}\x00(\x2C|\x21)/s', $data)) {
            ...
            if (THUMB_BROWSER_CACHE) {
                ...
                header('Expires: ' . gmdate('D, d M Y H:i:s T', strtotime('+30 days')));
            } else {
    ...
    switch ($type) {
        ...
        case 3:
            ...
            $pngQuality = (PNG_QUALITY - 100) / 11.111111;
            $pngQuality = round(abs($pngQuality));
            imagepng($im, $file_temp, $pngQuality);
            break;
    }
    imagedestroy($im);
    imagedestroy($oi);
...
header('Accept-Ranges: none');
header('Cache-Control: max-age=604800, must-revalidate');
header('Expires: ' . gmdate('D, d M Y H:i:s T', strtotime('+365 days')));
