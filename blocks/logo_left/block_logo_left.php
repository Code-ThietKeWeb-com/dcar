<?php
$vnT_Block = new block_logo_left ;
class block_logo_left extends Blocks {
  var $cache= true;
	 //
	function get_title() {
		global $conf,$vnT,$input;
   	$this->title = $vnT->lang['logo_left']['f_title'];
		return $this->title;
  }
	...
	function get_content() {
		...
		//check 
		$res_ck = $DB->query("select * from ad_pos where name='left'");
		...
		$result = $vnT->DB->query("select * from advertise where  display=1  and lang='$vnT->lang_name'  and  pos='left'  and  (FIND_IN_SET('$module',module_show) or (module_show='') )  order by type_ad DESC,l_order   ");
